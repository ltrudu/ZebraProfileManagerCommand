    ...
    private void waitForEMDK()
    {
        ...
        Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                long delta = 0;
                while(mEMDKManager == null || delta < Config.MAX_EMDK_TIMEOUT_IN_MS ) {
                    // Try to initialize EMDK
                    logMessage("Calling EMDK Initialization method", EMessageType.DEBUG);
                    initializeEMDK();
                    try {
                        logMessage("Waiting " + Config.WAIT_PERIOD_BEFORE_RETRY_EMDK_RETRIEVAL_IN_MS + " milliseconds before retrying.", EMessageType.DEBUG);
                        Thread.sleep(Config.WAIT_PERIOD_BEFORE_RETRY_EMDK_RETRIEVAL_IN_MS);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    delta = new Date().getTime() - startDate;
                    logMessage("Delta in ms since first EMDK retrieval try: " + delta + "ms stops at " + Config.MAX_EMDK_TIMEOUT_IN_MS + "ms", EMessageType.DEBUG);
                }
                logMessage("Could not retrieve EMDK Manager after waiting " + Config.WAIT_PERIOD_BEFORE_RETRY_EMDK_RETRIEVAL_IN_MS/Config.SEC_IN_MS + " seconds. Please contact your administrator or check logcat for any EMDK related error.", EMessageType.ERROR);
                bInitializing = false;
            }
        });
